...
export default async function ProfilePage() {
  ...
  const isNewUser =
    stats.totalCards === 0 &&
    trackProgress.every((t) => t.lessonsCompleted === 0);
    ...
    <div className="px-6 py-8 max-w-[960px] mx-auto space-y-5">
      ...
      {isNewUser ? (
        <>
          {/* Onboarding prompt */}
          <Link
            href="/learn?track=ear_training"
            className="block p-5 sm:p-6 rounded-xl border border-violet/20 bg-gradient-to-br from-violet/10 to-transparent hover:from-violet/15 transition-all"
          >
            <div className="text-[15px] font-bold text-ivory font-body">
              Start your first lesson to begin building your ear.
            </div>
            <div className="text-[12px] text-ash font-mono mt-1">
              The Ear Training track is a good place to start.
            </div>
            <div className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-body font-semibold text-violet">
              Begin Ear Training
              <span aria-hidden="true">&rarr;</span>
            </div>
          </Link>

          {/* Track selector with "Start here" hint */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {trackProgress.map((track) => (
                <Link
                  ...
                  className={`rounded-xl border bg-obsidian p-3.5 transition-colors ${
                    track.slug === "ear_training"
                      ? "border-violet/40 ring-1 ring-violet/20"
                      : "border-steel hover:border-violet/30"
                  }`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    {track.slug === "ear_training" && (
                      <span className="text-[9px] font-mono text-violet bg-violet/10 px-1.5 py-0.5 rounded shrink-0">
                        Start here
                      </span>
                    )}
                  </div>
                  <div className="text-[10px] text-ash font-mono mt-2">
                    {track.lessonsTotal} lessons
                  </div>
                </Link>
              ))}
            </div>
          ...
          {/* Compact radar with note */}
          <SkillRadar
            current={radarScores.current}
            lifetime={radarScores.lifetime}
            emptyMessage="Your skills radar will fill in as you learn and review"
          />
        </>
      ) : (
        <>
          ...
          >
            ...
            <div className="text-[12px] text-ash font-mono mt-0.5">
              {suggestion.subtitle}
            </div>
              ...
              {trackProgress.map((track) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-ash font-mono mt-1.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatCard
              ...
              sub={
                stats.dueToday > 0 ? `${stats.dueToday} remaining` : "all done"
              }
              color={colors.violet}
            />
            <StatCard
            ...
            <StatCard
              label="Mastered"
              value={
                stats.byStage.find((s) => s.stage === "mastered")?.count ?? 0
              }
          ...
          {recommendations.length > 0 && (
            <div className="rounded-xl border border-steel bg-obsidian p-5">
              <div className="text-[10px] tracking-[1.5px] uppercase text-ash font-mono mb-3.5">
            ...
            {weeklySummary.activityCount > 0 ? (
              <div className="flex gap-4 mb-4 text-sm">
                <div>
                  ...
                  <span className="text-ash ml-1">min</span>
                </div>
                <div>
                  <span className="text-ivory font-bold">
                    {weeklySummary.activityCount}
                  </span>
                  <span className="text-ash ml-1">activities</span>
                </div>
              </div>
            ) : null}
