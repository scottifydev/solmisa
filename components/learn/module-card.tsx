...
> = {
  ...
  in_progress: { borderClass: "border-coral/40", opacityClass: "" },
...
function StatusIcon({ status }: { status: ModuleProgressStatus }) {
  if (status === "locked") {
    return (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-ash">
  ...
  if (status === "completed") {
    return (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-correct">
  ...
  // available
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-silver">
...
export function ModuleCard({
  ...
  const color =
    degreeColors[module.module_order as keyof typeof degreeColors] ?? colors.coral;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-display text-lg text-ivory font-bold">{module.title}</h3>
          {module.description && (
            <p className="text-sm text-silver mt-1 line-clamp-2">{module.description}</p>
          )}
        </div>
