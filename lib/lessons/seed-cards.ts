...
export async function seedCardsFromLesson(
  input: SeedCardsInput,
): Promise<SeedCardsOutput> {
  ...
  if (!templates || templates.length === 0)
    return { cards_seeded: 0, cards_skipped: 0 };
  ...
  for (const result of seedable) {
    ...
    const { data: instance, error: instanceError } = await supabase
      ...
      .insert({
        ...
        prompt_rendered: result.seeds_card ?? "",
        answer_data: {},
        instance_parameters: {},
      })
      .select("id")
    ...
    const { error: stateError } = await supabase.from("user_card_state").upsert(
      {
        ...
        correct_streak: result.correct ? 1 : 0,
        total_reviews: 1,
