...
export async function evaluateCrossTopicUnlocks(
  userId: string,
  masteredChainTopic: string,
  masteredPosition: number,
): Promise<string[]> {
  const supabase = await createClient();

  // Find chains with cross_topic_mastery unlock condition
  const { data: crossChains } = await supabase
    .from("chain_definitions")
    .select("id, slug, unlock_condition")
    .eq("is_published", true)
    .eq("unlock_condition->>type", "cross_topic_mastery");

  if (!crossChains || crossChains.length === 0) return [];

  const activated: string[] = [];

  for (const chain of crossChains) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const condition = chain.unlock_condition as any;
    if (condition?.required_topic !== masteredChainTopic) continue;
    if (masteredPosition < (condition?.min_link ?? 3)) continue;
